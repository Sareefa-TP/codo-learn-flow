...
interface FeedbackEntry {
  id: string;
  type: "to_intern" | "to_mentor" | "from_intern" | "from_mentor";
  author: string;
  rating?: number;
  message: string;
  date: string;
...
const CoordinatorFeedback = () => {
  ...
  const [activeTab, setActiveTab] = useState("to-intern");
  ...
  const handleSubmit = () => {
    ...
    const newEntry: FeedbackEntry = {
      ...
      type: currentType as FeedbackEntry["type"],
      author: selectedUser || (currentType.includes("intern") ? "Alex Rivera" : "John Doe"),
      rating,
      message,
  ...
  const isOutgoingTab = activeTab === "to-intern" || activeTab === "to-mentor";
    ...
    <div className="animate-fade-in max-w-6xl mx-auto px-4 md:px-6 lg:px-8 pb-20">
      ...
      <div className="bg-slate-50/50 p-1.5 rounded-[1.5rem] border border-border/40 mb-8 w-full">
        <div className="grid grid-cols-4 gap-1 sm:gap-2">
          {[
            { id: "to-intern", label: "To Intern" },
            { id: "to-mentor", label: "To Mentor" },
            { id: "from-intern", label: "From Intern" },
                  ...
                  <CardContent className="p-4 sm:p-5 flex flex-col md:flex-row md:items-center justify-between gap-6">
                    ...
                    <div className="flex items-center gap-3 w-48 shrink-0">
                      <div className="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center text-primary shrink-0 transition-transform group-hover:scale-110 duration-500">
                        {entry.type.includes("intern") ? <UserCircle2 className="w-5 h-5" /> : <Users2 className="w-5 h-5" />}
                      </div>
                      <div className="min-w-0">
                        <p className="text-[10px] font-semibold text-muted-foreground uppercase tracking-tight">Name</p>
          ...
          >
            ...
            <form className="flex-1 overflow-y-auto p-6 space-y-6">
              ...
              <div className="space-y-4 p-4 rounded-xl bg-muted/30 border border-border/40">
                ...
                {/* Select User (Intern / Mentor) */}
                <div className="space-y-2">
                  <Label className="text-sm font-semibold text-foreground">
                    Select {activeTab.includes("intern") ? "Intern" : "Mentor"} <span className="text-red-500">*</span>
                  </Label>
                  <div className="relative">
                    <select
                      value={selectedUser}
                      ...
                      className="w-full h-10 rounded-xl border border-border/50 bg-background px-3 py-2 text-sm font-medium appearance-none cursor-pointer focus:outline-none focus:ring-2 focus:ring-primary/40 pr-10"
                    >
                      <option value="" disabled>Select {activeTab.includes("intern") ? "an intern" : "a mentor"}</option>
                      {activeTab.includes("intern") ? (
                        <>
            ...
            <div className="p-6 border-t border-border/50 bg-muted/5 flex items-center justify-end gap-3 mt-auto">
              ...
              <Button 
                ...
                disabled={!message.trim() || rating === 0 || !selectedCourse || !selectedUser}
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="flex items-center gap-2">
                      <div className="w-6 h-6 rounded-full bg-primary/10 flex items-center justify-center text-primary">
                        {selectedFeedback.type.includes("intern") ? <UserCircle2 className="w-4 h-4" /> : <Users2 className="w-4 h-4" />}
                      </div>
