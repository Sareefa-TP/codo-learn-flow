...
interface FeedbackEntry {
  id: string;
  type: "to_student" | "to_tutor" | "from_student" | "from_tutor";
  author: string;
  rating?: number;
  message: string;
  date: string;
...
const mockFeedback: FeedbackEntry[] = [
  {
    ...
    rating: 5,
    message: "Thank you for the guidance on my ML project. Your feedback was very helpful!",
    date: "2024-03-25",
    status: "Read"
  },
  {
    ...
    rating: 4,
    message: "Good work on the recent student evaluation. Focus more on communication skills.",
    date: "2024-03-24",
    status: "Read"
  },
  {
    ...
    author: "You",
    rating: 5,
    message: "Excellent progress on the Dashboard project. Your attention to detail is impressive.",
    date: "2024-03-20"
  },
  {
    id: "4",
    ...
    author: "You",
    rating: 4,
    message: "The student progress reports are looking better. Let's focus on the next phase.",
    date: "2024-03-19"
  }
];
...
const MentorFeedback = () => {
  ...
  const [activeTab, setActiveTab] = useState("to-student");
  ...
  const tabMapping: Record<string, FeedbackEntry["type"]> = {
    "to-student": "to_student",
    "to-tutor": "to_tutor",
    "from-student": "from_student",
    "from-tutor": "from_tutor",
  };
  ...
  const handleSubmit = () => {
    ...
    if (!selectedUser) {
      toast.error(`Please select a ${activeTab.includes("student") ? "student" : "tutor"}`);
      return;
    }
    ...
    <>
      <DashboardLayout>
        <div className="animate-fade-in max-w-6xl mx-auto px-4 md:px-6 lg:px-8 pb-20 mt-4">
          ...
          <div className="flex items-center gap-3 mb-10 animate-in fade-in slide-in-from-top-4 duration-700 delay-100">
            <PageSearch
          ...
          <div className="bg-slate-50/50 p-1.5 rounded-[1.5rem] border border-border/40 mb-10 w-full mx-auto">
            <div className="grid grid-cols-4 gap-1 sm:gap-2">
              {[
                { id: "to-student", label: "To Student" },
                { id: "to-tutor", label: "To Tutor" },
                { id: "from-student", label: "From Student" },
                { id: "from-tutor", label: "From Tutor" },
              ].map((tab) => (
                <button
                  key={tab.id}
                  onClick={() => setActiveTab(tab.id)}
                  className={cn(
                    "px-1 sm:px-4 py-3 rounded-2xl text-[8px] sm:text-[10px] md:text-xs font-bold uppercase tracking-widest transition-all duration-300 text-center flex items-center justify-center h-full",
                    activeTab === tab.id 
                      ...
                      <CardContent className="p-4 sm:p-5 flex flex-col md:flex-row md:items-center justify-between gap-6">
                        ...
                        <div className="flex items-center gap-3 w-48 shrink-0">
                          <div className="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center text-primary shrink-0 transition-transform group-hover:scale-110 duration-500">
                             {entry.type.includes("student") ? <Users2 className="w-5 h-5" /> : <UserCircle2 className="w-5 h-5" />}
                          </div>
                        ...
                        <div className="flex-1 min-w-0 space-y-2">
                          ...
                          <div className="relative">
                            ...
                            <p className="text-sm text-foreground/80 leading-relaxed italic pl-3 relative z-10">
                              {entry.message}
                            </p>
                          </div>
                        </div>
            ...
            >
              ...
              <div className="flex items-center justify-between p-6 border-b border-border/50 bg-muted/5">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground mt-1">
                    Share your evaluation with the {activeTab.includes("student") ? "student" : "tutor"}.
                  </p>
                </div>
                <button 
              ...
              <form className="flex-1 overflow-y-auto p-6 space-y-6">
                <div className="space-y-4 p-4 rounded-xl bg-muted/30 border border-border/40">
                  ...
                  <div className="space-y-2">
                    <Label className="text-sm font-semibold text-foreground">
                      Select {activeTab.includes("student") ? "Student" : "Tutor"} <span className="text-red-500">*</span>
                    </Label>
                    <div className="relative">
                      <select
                        value={selectedUser}
                      ...
                      >
                        <option value="" disabled>Select a {activeTab.includes("student") ? "student" : "tutor"}</option>
                        {activeTab.includes("student") ? (
                          <>
                            <option value="diya">Diya Krishnan</option>
                            <option value="meera">Meera Bhat</option>
                            <option value="varun">Varun Choudhary</option>
                          </>
                        ) : (
                          <>
                ...
                <div className="space-y-4 p-4 rounded-xl bg-muted/30 border border-border/40">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1">
                      ...
                      <div className="flex items-center gap-2">
                         {selectedFeedback.type.includes("student") ? <Users2 className="w-4 h-4 text-primary" /> : <UserCircle2 className="w-4 h-4 text-primary" />}
