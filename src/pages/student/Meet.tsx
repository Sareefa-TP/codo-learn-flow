        ...
        <div className="p-5 flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3 self-end md:self-center">
            ...
            {session.status === "live" && (
              <Button 
                size="sm" 
                className="h-10 text-xs font-bold rounded-lg px-8 bg-primary hover:bg-primary/90 shadow-md shadow-primary/20 animate-in zoom-in-95 duration-300"
        ...
        <div className="p-5 flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3 self-end md:self-center">
            {session.isActive ? (
              <Button 
                size="sm" 
                className="h-10 text-xs font-bold rounded-lg px-8 bg-primary hover:bg-primary/90 shadow-md shadow-primary/20"
                onClick={() => window.open(session.meetLink, '_blank')}
              >
                Join Now
              </Button>
...
const StudentMeet = () => {
  ...
  useEffect(() => {
    if (!tab) {
      navigate("/student/meet/live", { replace: true });
    }
  }, [tab, navigate]);

  const handleTabChange = (value: string) => {
    navigate(`/student/meet/${value}`);
  };
