      ...
      <div className="animate-fade-in space-y-6 max-w-[1200px] mx-auto pb-10 px-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
               <h1 className="text-2xl font-bold tracking-tight text-slate-900">Schedule New Session</h1>
           ...
           <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
              ...
              >
                  ...
                  >
                    ...
                    <span className="text-xs font-black uppercase">Batch-based</span>
                  </Label>
                  <Label 
                    className={cn(
                      "flex items-center gap-2.5 px-6 py-3 rounded-xl border-2 transition-all cursor-pointer",
                  ...
                  >
                    ...
                    <span className="text-xs font-black uppercase">Custom Roster</span>
                  </Label>
              </RadioGroup>
           </div>
        </Card>
        ...
        <form onSubmit={handleCreate} className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-4 space-y-8">
            ...
            <Card className="border-none shadow-sm rounded-2xl overflow-hidden bg-white ring-1 ring-slate-100">
               ...
               <CardContent className="p-8 space-y-6">
                <div className="space-y-3">
                  ...
                  >
                    <SelectTrigger className="h-12 bg-slate-50/50 border-slate-100/50 rounded-xl font-bold text-slate-900 shadow-none">
            ...
            {/* SCHEDULE CONFIG */}
            <Card className="border-none shadow-sm rounded-2xl overflow-hidden bg-white ring-1 ring-slate-100 font-mono">
               <div className="px-8 py-5 border-b border-slate-50 flex items-center gap-3">
                  <div className="p-2 bg-pink-50 rounded-lg"><Clock className="w-3.5 h-3.5 text-pink-600" /></div>
                  <h3 className="text-[10px] font-black font-sans uppercase tracking-widest text-slate-500">Timeline</h3>
               </div>
               <CardContent className="p-8 space-y-6">
                <div className="space-y-3">
                  <Label className="text-[10px] font-black font-sans uppercase tracking-widest text-slate-500 opacity-60">Session Date <span className="text-red-500">*</span></Label>
                  <Input 
                    type="date"
                    value={formData.date}
                    onChange={(e) => setFormData({ ...formData, date: e.target.value })}
                    className="h-12 bg-slate-50/50 border-slate-100/50 rounded-xl font-bold text-slate-900 shadow-none" 
                  />
                </div>
                <div className="space-y-3">
                  <Label className="text-[10px] font-black font-sans uppercase tracking-widest text-slate-500 opacity-60">Start Time <span className="text-red-500">*</span></Label>
                  <Input 
                    type="time"
                ...
                <div className="space-y-3">
                  <Label className="text-[10px] font-black font-sans uppercase tracking-widest text-slate-500 opacity-60">Total Duration</Label>
                  <Select value={formData.duration} onValueChange={(v) => setFormData({ ...formData, duration: v })}>
                    ...
                    <SelectContent className="rounded-xl border-none shadow-xl">
                      ...
                      <SelectItem value="60">1.0 Hour</SelectItem>
                      <SelectItem value="90">1.5 Hours</SelectItem>
                      <SelectItem value="120">2.0 Hours</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
               </CardContent>
            ...
            <Card className="border-none shadow-sm rounded-2xl overflow-hidden bg-white ring-1 ring-slate-100">
               <div className="px-8 py-5 border-b border-slate-50 flex items-center gap-3">
                  ...
                  <h3 className="text-[10px] font-black uppercase tracking-widest text-slate-500">Connectivity</h3>
               </div>
               <CardContent className="p-8 space-y-4">
                  <div className="space-y-3">
                    <Label className="text-[10px] font-black uppercase tracking-widest text-slate-500 opacity-60">Meeting Link <span className="text-red-500">*</span></Label>
                    <div className="relative">
                      <Link2 className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-300" />
                      <Input 
                        value={formData.link}
                        onChange={(e) => setFormData({ ...formData, link: e.target.value })}
                        className="pl-10 h-13 bg-slate-50/50 border-slate-100/50 rounded-xl font-bold text-slate-900 placeholder:text-slate-300 placeholder:font-normal" 
            ...
            <Card className="border-none shadow-sm rounded-2xl overflow-hidden bg-white ring-1 ring-slate-100">
               <div className="px-8 py-5 border-b border-slate-50 flex items-center gap-3">
                  ...
                  <h3 className="text-[10px] font-black uppercase tracking-widest text-slate-500">Preferences</h3>
               </div>
               <CardContent className="p-8 space-y-8">
                  <div className="flex items-center justify-between">
                     <div className="space-y-0.5">
                        <Label className="text-xs font-bold text-slate-800">Recording</Label>
                        <p className="text-[10px] font-medium text-slate-400">Store video session</p>
                     </div>
                     <Switch checked={formData.recording} onCheckedChange={(v) => setFormData({ ...formData, recording: v })} />
                  </div>
                  <div className="flex items-center justify-between">
                     <div className="space-y-0.5">
                        <Label className="text-xs font-bold text-slate-800">Public Chat</Label>
                        <p className="text-[10px] font-medium text-slate-400">Enable session messaging</p>
                     </div>
                     <Switch checked={formData.chat} onCheckedChange={(v) => setFormData({ ...formData, chat: v })} />
                  </div>
                  <div className="space-y-3 pt-2">
                    <Label className="text-[10px] font-black uppercase tracking-widest text-slate-500 opacity-60">Push Notification</Label>
                    <Select value={formData.reminder} onValueChange={(v) => setFormData({ ...formData, reminder: v })}>
                      <SelectTrigger className="h-11 bg-slate-50/50 border-slate-100/50 rounded-xl font-bold text-slate-900">
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent className="rounded-xl border-none shadow-xl">
                        <SelectItem value="5" className="font-bold py-2 rounded-lg">5 min before</SelectItem>
                        <SelectItem value="15" className="font-bold py-2 rounded-lg">15 min before</SelectItem>
                        <SelectItem value="30" className="font-bold py-2 rounded-lg">30 min before</SelectItem>
                      </SelectContent>
                    </Select>
                  </div>
               </CardContent>
...
export default AdminCreateMeet;
