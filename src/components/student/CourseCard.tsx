  ...
  return (
    <Card className="group overflow-hidden border-border/50 hover:border-primary/20 hover:shadow-2xl hover:shadow-primary/5 transition-all duration-500 rounded-3xl flex flex-col h-full">
      ...
      <CardContent className="p-4 sm:p-6 flex-1 flex flex-col space-y-5 sm:space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4 pt-4 mt-auto">
          <Button 
            variant="ghost" 
            size="sm" 
          ...
          <Button 
            className="bg-primary hover:bg-primary/90 gap-2 rounded-xl h-10 font-bold text-xs shadow-md shadow-primary/20 w-full sm:w-auto"
            onClick={(e) => {
              e.stopPropagation();
              onActionClick?.();
            }}
